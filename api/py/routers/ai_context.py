...
def build_context(df: pd.DataFrame, symbol: str, source: str = "VCI") -> str:
    ...
    ret5 = (
        (v("close") / df.iloc[-5]["close"] - 1) * 100 if len(df) >= 5 else float("nan")
    )
    ret20 = (
        (v("close") / df.iloc[-20]["close"] - 1) * 100
        if len(df) >= 20
        else float("nan")
    )
    ...
    rsi_lbl = (
        "Quá mua" if v("rsi") > 70 else ("Quá bán" if v("rsi") < 30 else "Trung tính")
    )
    ...
    for _, r in tbl.sort_values("time", ascending=False).iterrows():
        rsi_val = (
            float(r["rsi"])
            if ("rsi" in r.index and r["rsi"] == r["rsi"])
            else float("nan")
        )
        macdh = (
            float(r["macdHist"])
            if ("macdHist" in r.index and r["macdHist"] == r["macdHist"])
            else float("nan")
        )
        ...
        if hasattr(q, "foreign_flow"):
            ...
            if ff_df is not None and not ff_df.empty:
                ...
                for _, r in ff_df.head(5).iterrows():
                    rows_ff.append(
                        f"- Ngày {r.get('date', '—')}: Ròng {r.get('net_value', 0):+,.0f} VNĐ (Mua: {r.get('buy_value', 0):,.0f} | Bán: {r.get('sell_value', 0):,.0f})"
                    )
                foreign_md = (
    ...
    if not foreign_md:
        try:
            ...
            if board_df is not None and not board_df.empty:
                ...
                sell_vol = board_df.get("foreign_sell_volume", pd.Series([None])).iloc[
                    0
                ]
                if (
                    buy_vol is not None
                    and sell_vol is not None
                    and not (pd.isna(buy_vol) or pd.isna(sell_vol))
                ):
                    net_vol = buy_vol - sell_vol
                    net_flow_vnd = ""
                    close_price = (
                        board_df.get("close_price", pd.Series([None])).iloc[0]
                        or board_df.get("reference_price", pd.Series([None])).iloc[0]
                    )
                    if close_price:
                        try:
