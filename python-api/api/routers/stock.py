"""
GET /stock/history   → OHLCV history
GET /stock/realtime  → latest quote
POST /stock/board    → batch real-time board via Trading.price_board()
"""
import re
...
def get_board(req: BoardRequest):
    ...
    try:
        ...
        # Trading requires symbol= in __init__ (any valid symbol works)
        ...
        # Flatten MultiIndex or normalize flat columns
        ...
        for row in records:
            # KBS: symbol  |  VCI MultiIndex flattened: listing_symbol
            sym = str(
            ...
            if req.source == "KBS":
                # KBS flat — prices already in VND
                ...
                # VCI MultiIndex flattened: listing_ref_price, match_open_price, etc.
                open_ = _safe_float(
            ...
            # Convert to VND if prices are in points (< 10,000)
            if 0 < close < 10_000:
